...
- [x] Реализовать обработчик страницы альбома в `handlers.go`
- [x] Создать функцию получения списка изображений пользователя по cookie
- [x] Реализовать отображение изображений в сетке на странице альбома
- [x] Добавить возможность копирования URL изображений
- [x] Добавить навигацию между страницами при большом количестве изображений


## Фаза 7: Реализация системы очистки

- [x] Реализовать логику очистки старых изображений в `cleanup.go`
- [x] Определить что qualifies как "старое" изображение (время последнего доступа > 24 часа)
- [x] Запустить очистку периодически в фоновой goroutine с использованием time.Ticker
- [x] Удалять пустые директории после очистки файлов
- [x] Реализовать graceful shutdown для процесса очистки при отмене контекста приложения
